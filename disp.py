...
def det(u0,*arg):
  ...
  if(theta>0.1): # oblique propagation
      ...
      for i in range(int(p['Nsp'][0])):
        ...
        kVOmp = wave_k*p['V'][i]/np.sqrt(p['bperp'][0])
        kpwp  = wave_k*np.sqrt(p['bpara'][i]/p['bperp'][0]*p['mu'][i])
        pref  = p['mu'][i]*p['den'][i]
        ze    = (omega-kVOmp+pol*p['Omega'][i])/kpwp
        ze0   = (omega-kVOmp)/kpwp
        res  += (ze0*f.Z(ze) - delta*f.dp(ze,0)/2.)*pref
        #res  += pref*(delta-1.+(delta*(omega-kVOmp)+pol*
        #        (delta-1.)*p['Omega'][i]))*f.Z(ze)/kpwp
